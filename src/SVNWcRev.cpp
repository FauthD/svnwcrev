...
#define USE_TIME_NOW    apr_time_now()
...
void SecureZeroMemory(void *pBuf, size_t size)
{
	if(pBuf!=NULL)
	{
		memset(pBuf, 0, size);
				...
				apr_time_t date_svn)
{
    ...
    if (strstr(def, "UTC"))
    {
    	status = apr_time_exp_gmt(&newtime, date_svn);
    }
    else
    {
    	status = apr_time_exp_lt(&newtime, date_svn);
    }
	if(status)
		return false;
	        ...
	        (strcmp(def,DATEWFMTDEFUTC) == 0) || (strcmp(def,NOWWFMTDEFUTC) == 0) || (strcmp(def,LOCKWFMTDEFUTC) == 0))
	{
		...
		{
			pEnd++;
			if (pEnd - pBuf >= (int64_t)filelength)
...
//		_invalid_parameter_handler oldHandler = _set_invalid_parameter_handler(_invalid_parameter_donothing);
		// not sure how this can be done in Linux and do we need it at all?

		apr_size_t RetSize=0;
		if (apr_strftime(destbuf,&RetSize,1024,format,&newtime) == 0)
		{
			if (errno == EINVAL)
				strcpy(destbuf, "Invalid Time Format Specified");
		}
...
// This only works with SVN headers of version 1.3.x or above
	...
	/* This is a work around to the issues with non-working relative paths (..).
	 * The error must be somewhere in status.cpp, but until we locate it,
	 * we simply translate into an absolute path.
	 * */
	char resolved_path[MAX_PATH+1];
	const char *abspath=NULL;
	abspath = realpath(wc, resolved_path);
	if(abspath)
		wc = abspath;
